            ...
            for event in events:
                if event.name in interaction.channel.name:
                    if disabled_text in event.description:
                        ...
                        await log(client=self.client, content=f"<@{interaction.user.id}> enabled invites for the event `{event.name}`")
                    else:
                        await event.edit(description=event.description + disabled_text)
                        await interaction.response.send_message(content="Done.", ephemeral=True)
                        await log(client=self.client, content=f"<@{interaction.user.id}> disabled invites for the event `{event.name}`")
                    return
